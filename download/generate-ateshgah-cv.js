...
// ── Minimalist palette (same as Kontakt Home CV) ──
const C = {
  name: "1A1A1A",
  accent: "2A6496",
  title: "1A1A1A",
...
// ── Section heading with left accent line ──
...
function experienceHeader(title, company, dateRange) {
  return new Paragraph({
    ...
    children: [
      new TextRun({ text: title, font: FONT, size: 21, bold: true, color: C.title }),
      new TextRun({ text: "  |  ", font: FONT, size: 19, color: C.sec }),
      new TextRun({ text: company, font: FONT, size: 19, color: C.body, italics: true }),
...
children.push(
  new Paragraph({
    ...
    tabStops: [{ type: TabStopType.RIGHT, position: 9506 }],
    children: [
      new TextRun({ text: "Bachelor of Science in Computer Science", font: FONT, size: 20, bold: true, color: C.title }),
      new TextRun({ text: "\t", font: FONT }),
      new TextRun({ text: "【University Name】, Baku", font: FONT, size: 18, color: C.sec }),
    ],
  })
);
